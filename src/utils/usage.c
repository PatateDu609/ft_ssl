...
static int get_longest(const struct s_command *cmd)
{
	...
	else if (!longest)
	{
		...
		{
			int len = strlen(cmd->options[i]->name) + OPT_PREFIX_LEN;
		...
		{
			int len = strlen(cmd->params[i].name);
			if (longest < len)
				longest = len;
		}
	}
...
static void ft_print(struct s_option **options, enum e_opt_type type)
{
	...
	for (int i = 0; options[i]; i++)
	{
		...
		{
			int len = longest - strlen(options[i]->name) - 1;
			fprintf(stderr, "%s-%s%s  %s%-*s%s",
				COLOR_RED COLOR_BOLD, options[i]->name, COLOR_RESET,
				COLOR_BLUE, len, options[i]->val_desc, COLOR_RESET);
		}
		else
			fprintf(stderr, "%s-%-*s%s", COLOR_RED COLOR_BOLD, longest + 1, options[i]->name, COLOR_RESET);
